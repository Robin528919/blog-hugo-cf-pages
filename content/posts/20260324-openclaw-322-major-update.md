...
> 本文基于 OpenClaw v2026.3.22 / v2026.3.23，适用于 Node.js 24（22.16+ LTS 兼容）。
...
![多平台体验升级一览](/images/openclaw-322-major-update/platform-channels.svg)
...
![Agent 引擎核心升级](/images/openclaw-322-major-update/agent-engine.svg)
...
- **插件断代迁移成本**：旧 API 一刀切无兼容层，存量第三方插件必须手动迁移 import 路径，短期内可能导致部分插件生态断裂
- **Beta 版风险**：3.22 首发为 beta 版本，虽然同日推出正式版和 3.23 修复版，但生产环境建议等一周观察社区反馈再升级
- **ClawHub 生态尚在早期**：官方首选分发渠道刚上线，包数量远不及 npm，冷门插件可能仍需回退到 npm 安装
- **48 小时超时的代价**：长时间运行的 Agent 会话会持续占用服务器资源，自托管用户需注意内存和连接数规划
